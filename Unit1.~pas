...
begin
rez:=true;
if ((ord(adressString[iGraph+1])>=48)and
  (ord(adressString[iGraph+1])<=57)) then
  begin
    if ((strToInt(adressString[iGraph+1])>8)or
      (strToInt(adressString[iGraph+1])<1))then
      begin
        rez:=false;
      end;
  end
...
if numOutElemG<>-1 then
  begin
    ...
    form1.Memo1.Lines.Add('Номера точек в кадре ОРБИТА'+
    ...
    if i>masgMax-1 then
      begin
        ...
        while iGr<=MAXGROUPINKADR do
          begin
            inc(j);
            form1.Memo1.Lines.Add('Группа №'+intToStr(iGr)+
              ' '+intToStr(j)+' слово. '+intToStr(i));
            iGr:=iGr+(trunc(stepOutG/masgMax));
          end;

        //form1.Memo1.Lines.Add('В текущей группе значения нет'+
          //'возможно он есть в следующих группах');
...
form1.Label1.Caption:='Проверяемый адреc Орбита '+
  form1.ComboBox1.Items[form1.ComboBox1.itemindex];
end;
